...
function generateLargeContent(wordCount: number = 1000): any[] {
  const words = [
    'The', 'argument', 'presents', 'a', 'logical', 'fallacy', 'that', 'undermines',
    'the', 'validity', 'of', 'conclusion', 'because', 'it', 'relies', 'on',
    'premises', 'which', 'are', 'not', 'necessarily', 'true', 'or', 'relevant',
    'to', 'central', 'claim', 'being', 'made', 'by', 'speaker', 'in', 'this',
    'debate', 'context', 'we', 'must', 'consider', 'evidence', 'carefully',
  ];
...
function generateAnnotations(count: number = 50): Record<string, any> {
  const fallacyTypes = [
    ...
    'appeal-to-authority', 'hasty-generalization', 'red-herring',
    'circular-reasoning', 'bandwagon', 'false-cause',
  ];
...
describe('Performance Benchmarks', () => {
  ...
  describe('Large Document Performance', () => {
    it('should handle document with 1000+ words', async () => {
      ...
      const wordCount = largeContent.reduce((sum, p) => 
        sum + (p.children[0]?.text?.split(' ').length || 0), 0
      );
  ...
  describe('Memory and Stress Tests', () => {
    it('should handle rapid successive operations', async () => {
      ...
      console.log(`${operations * 3} operations in ${duration.toFixed(2)}ms (avg: ${avgPerOperation.toFixed(2)}ms)`);
